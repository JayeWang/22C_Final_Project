//
//  Created by Zhou Wang, revised by Jakin and Yunyi
//  Student.hpp
//  TempProject
//
////////////
...
public:
   Student(){studentID=""; Name=""; string enroll_Date=""; status=0; GPA=0; unitCompleted=0;};
...
Student Student::operator = (const Student& theStudent){
   Student std;
   std.studentID = theStudent.studentID;
   std.enroll_Date = theStudent.enroll_Date;
   std.unitCompleted = theStudent.unitCompleted;
   std.GPA = theStudent.GPA;
   std.status = theStudent.status;
   std.Name = theStudent.Name;
   return std;
}
